...
var (
	...
	kubeCronjobSkippedJobsTotal = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "kube_cronhpa_skipped_jobs_total",
			Help: "Skipped jobs per deployment",
		},
		[]string{"namespace", "cronhpa", "job"},
	)
)
...
func init() {
	...
	metrics.Registry.MustRegister(kubeCronjobSkippedJobsTotal)
}
...
func IncSkipJob(cronHpa *v1beta1.CronHorizontalPodAutoscaler, job v1beta1.Job) {
	kubeCronjobSkippedJobsTotal.WithLabelValues(cronHpa.Namespace, cronHpa.Name, job.Name).Inc()
}
